...
codons = list(map(''.join, zip(*[iter(complement_sequence_to_read)] * 3)))  # sequence 문자열을 3개씩 잘라서 codon 리스트에 넣는다.
...
if len(amino_acids) != len(codons):  # 아미노산이 들어있는 리스트의 요소 개수와 codons가 들어있는 리스트의 요소 개수가 일치하면
    ...
    for i in range(0, len(codons), 20):
        if i + 20 < len(codons):  # codons 개수가 i+20보다 많을 때
            list_r1.append(amino_acids[i : i + 20])
        else:  # codons 개수가 i+20보다 적을 때 뒤의 범위를 비워, 한 줄에 있는 아미노산 개수가 20개보다 작아도 출력되도록 한다.
            list_r1.append(amino_acids[i::])
    list_join_r1 = sum(list_r1, [])
    list_join2_r1 = '  '.join(list_join_r1)
    print(list_join2_r1[::-1], '\n')

# 상보적인 시퀀스를 reverse시켜 변수에 넣고, 시퀀스 문자열 리스트를 인덱스 1부터 읽는다. 아미노산을 세 개씩 또 다른 변수에 넣고 거꾸로 출력

codons = list(map(''.join, zip(*[iter(complement_sequence_to_read[1:])] * 3)))  # sequence 문자열을 3개씩 잘라서 codon 리스트에 넣는다.
...
if len(amino_acids) != len(codons):  # 아미노산이 들어있는 리스트의 요소 개수와 codons가 들어있는 리스트의 요소 개수가 일치하면
    ...
    for i in range(0, len(codons), 20):
        if i + 20 < len(codons):  # codons 개수가 i+20보다 많을 때
            list_r2.append(amino_acids[i : i + 20])
        else:  # codons 개수가 i+20보다 적을 때 뒤의 범위를 비워, 한 줄에 있는 아미노산 개수가 20개보다 작아도 출력되도록 한다.
            list_r2.append(amino_acids[i::])
    list_join_r2 = sum(list_r2, [])
    list_join2_r2 = '  '.join(list_join_r2)
    print(list_join2_r2[::-1], '\n')

# 상보적인 시퀀스를 reverse시켜 변수에 넣고, 시퀀스 문자열 리스트를 인덱스 2부터 읽는다. 아미노산을 세 개씩 또 다른 변수에 넣고 거꾸로 출력

codons = list(map(''.join, zip(*[iter(complement_sequence_to_read[2:])] * 3)))  # sequence 문자열을 3개씩 잘라서 codon 리스트에 넣는다.
...
if len(amino_acids) != len(codons):  # 아미노산이 들어있는 리스트의 요소 개수와 codons가 들어있는 리스트의 요소 개수가 일치하면
    ...
    for i in range(0, len(codons), 20):
        if i + 20 < len(codons):  # codons 개수가 i+20보다 많을 때
            list_r3.append(amino_acids[i : i + 20])
        else:  # codons 개수가 i+20보다 적을 때 뒤의 범위를 비워, 한 줄에 있는 아미노산 개수가 20개보다 작아도 출력되도록 한다.
            list_r3.append(amino_acids[i::])
    list_join_r3 = sum(list_r3, [])
    list_join2_r3 = '  '.join(list_join_r3)
    print(list_join2_r3[::-1], '\n')
